...
def ensure_local_path(path: str) -> str:
    ...
    if ((not RUNNING_IN_CODESPACES) and RUNNING_IN_DOCKER):
        print("IN HERE",
              RUNNING_IN_DOCKER)  # If absolute Docker path, return as-is :  # If absolute Docker path, return as-is
        return path
...
def format_file_with_prettier(file_path: str, prettier_version: str):
    ...
    Args:
        file_path: The relative path to the file to format.
        prettier_version: The version of Prettier to use.
    """
    # Ensure we're using a relative path inside the project
    project_root = os.path.dirname(os.path.abspath(__file__))  # Get script directory
    input_file_path = os.path.join(project_root, file_path)  # Convert to absolute path
    ...
    with open(input_file_path, "r") as file:
        for line in file:
            ...
            if date_component == 'weekday' and parsed_date.weekday() == target_value:
                ...
                count += 1
            elif date_component == 'leap_year' and parsed_date.year % 4 == 0 and (
...
def fetch_data_from_api_and_save(url: str, output_file: str, generated_prompt: str,
